...
# v3: printing out the members in order of occurence on the chr
...
foreach $i (@mf_itemsets_table){
	...
	# sorting the copies in ascending order
	@temp = sort {$a->[0] <=> $b->[0]} @copies;
	@copies=@temp;
...
# 		unless(open(OUTFILECOPIES100K,">".$fam1."_".$rel."_".$fam2.".100k.fas")){print "not able to open  ".$fam1."_".$rel."_".$fam2.".100k.fas\n\n";exit;}
	...
	foreach $k (@copies){# reusing $k
		...
		if ($extend){
			...
			if($k->[2] eq '+'){
				print OUTFILECOPIES1K substr($chr_seq,(($k->[0]-1)-1000),(($k->[1] - $k->[0])+2000)),"\n";
			}
			elsif ($k->[2] eq 'C'){
				print OUTFILECOPIES1K comp(substr($chr_seq,(($k->[0]-1)-1000),(($k->[1] - $k->[0])+2000))),"\n";
			}
			else{ # error catcher
				print STDERR "ERROR!! \n";
			}

...
# WG: Generate correct linkouts for WG gbrowse
#   : Create Gbrowse GFF file for ProxMiner families

# TODO: Computing the significance of each rel using chi square 
...
# calculate the significance at p=0.05 using chi square
sub chi_sq{
	my ($chisq,$ocount,$ecount,$f1count,$f2count);
	$ocount=$_[0];
	$ecount=$_[1];
	$f1count=$_[2];
	$f2count=$_[3];
...
# 	if(($i->[8] > $UD_conf_lim) && &chi_sq($i->[3],$i->[5],$i->[12],$i->[15]) && ($i->[1] =~ /^U/)){ # for U relationships 4 CHI SQ TEST
...
foreach $i(@UD_conn_comp){
	if ($#$i > 0){# for ccomps with >1 member
		print OUTFILEDATA "\n\# ProxMiner family :",$ctr,"\n";
		print OUTFILEHTML "<h2>ProxMiner family &nbsp : &nbsp ",$ctr++,"</h2>\n"; 
...
# 			$rep_fams{$j}=[@temp];
		...
		if(keys %CRF_annot_counts > 0){
			...
			print OUTFILEDATA "\n\# ProxMiner family annotation:",$temp[0],"\n";
...
# 				print OUTFILEDATA "$_ "; 
			...
			@temp=&resolv_coord($j->[0]);
			$start=$temp[1];
			@temp=&resolv_coord($j->[1]);
			$end=$temp[1];
			$k='Chr'.$temp[0];
			
			print OUTFILEHTML '<br>';
			print OUTFILEHTML 'Length : &nbsp',($j->[1]-$j->[0]),' bp <br>';
			print OUTFILEHTML 'Chromosome : &nbsp ',$temp[0],' <br>';
# 			print OUTFILEDATA "Exact\t\tStart:$j->[0]\tEnd:$j->[1]\n";
			print OUTFILEHTML "\nExact &nbsp &nbsp &nbsp &nbsp &nbsp Start:$start &nbsp End:$end &nbsp \t",'<a href="http://rice.plantbiology.msu.edu/cgi-bin/gbrowse/rice?name=',$k,':',$start,'..',$end,'">TIGR</a>';
			print OUTFILEGBGFF "chr$temp[0]\tProxMiner\trepeat_unit\t$start\t$end\t.\t+\t.\tFam PMfam=",$ctr-1,"\n";
			
			print OUTFILEHTML "\t",'<a href="http://130.18.208.245/cgi-bin/gbrowse/rice/?name=',$k,':',$start,'..',$end,'">CSE</a><br>';
			
			# with 0 flanking
			print OUTFILEHTML "\n1k flanking &nbsp Start:",$start-1000,"&nbsp End:",$end+1000,"&nbsp &nbsp\t",'<a href="http://rice.plantbiology.msu.edu/cgi-bin/gbrowse/rice?name=',$k,':',$start-1000,'..',$end+1000,'">TIGR</a>';
			print OUTFILEHTML "\t",'<a href="http://130.18.208.245/cgi-bin/gbrowse/rice/?name=',$k,':',$start-1000,'..',$end+1000,'">CSE</a><br>';
			
# 			print OUTFILEDATA "2k flanking\tStart:",$j->[0]-2000,"\tEnd:",$j->[1]+2000,"\n";
			print OUTFILEHTML "\n2k flanking &nbsp Start:",$start-2000,"&nbsp End:",$end+2000,"&nbsp &nbsp\t",'<a href="http://rice.plantbiology.msu.edu/cgi-bin/gbrowse/rice?name=',$k,':',$start-2000,'..',$end+2000,'">TIGR</a>';
			print OUTFILEHTML "\t",'<a href="http://130.18.208.245/cgi-bin/gbrowse/rice/?name=',$k,':',$start-2000,'..',$end+2000,'">CSE</a><br>';
			
# 			print OUTFILEDATA "4k flanking\tStart:",$j->[0]-4000,"\tEnd:",$j->[1]+4000,"\n";
			print OUTFILEHTML "\n4k flanking &nbsp Start:",$start-4000,"&nbsp End:",$end+4000,"&nbsp &nbsp\t",'<a href="http://rice.plantbiology.msu.edu/cgi-bin/gbrowse/rice?name=',$k,':',$start-4000,'..',$end+4000,'">TIGR</a>';
			print OUTFILEHTML "\t",'<a href="http://130.18.208.245/cgi-bin/gbrowse/rice/?name=',$k,':',$start-4000,'..',$end+4000,'">CSE</a><br>';
			
# 			print OUTFILEDATA "5k flanking\tStart:",$j->[0]-5000,"\tEnd:",$j->[1]+5000,"\n";
			print OUTFILEHTML "\n5k flanking &nbsp Start:",$start-5000,"&nbsp End:",$end+5000,"&nbsp &nbsp\t",'<a href="http://rice.plantbiology.msu.edu/cgi-bin/gbrowse/rice?name=',$k,':',$start-5000,'..',$end+5000,'">TIGR</a>';
			print OUTFILEHTML "\t",'<a href="http://130.18.208.245/cgi-bin/gbrowse/rice/?name=',$k,':',$start-5000,'..',$end+5000,'">CSE</a><br>';
...
print OUTFILEDATA "\n\# Number of ProxMiner families: $ctr\n";
print OUTFILEDATA "\# Number of ProxMiner families with only annotated RScout families: $fullyannotated\n";
print OUTFILEDATA "\# Number of ProxMiner families with some annotated RScout families: $partiallyannotated\n";
print OUTFILEDATA "\# Number of ProxMiner families with no annotated RScout families: $unannotated\n\n";
...
print OUTFILEDATA "\n\#For all ProxMiner families\n";
print OUTFILEDATA "\#Size\tNofPMfams\n";
...
$i= keys %rep_fams;
print OUTFILEDATA "\n\# Number of RScout families in repeat library: $i\n";
$ctr=$k=$temp=0;
@temp=();
...
print OUTFILEDATA "\# Number of RScout singletons: $ctr\n";
print OUTFILEDATA "\# Number of RScout families in ProxMiner families: $k\n";
print OUTFILEDATA "\# Number of RScout families in >1 ProxMiner families: $temp\n"; 
if(scalar @temp > 0){ 
	print OUTFILEDATA "\# RScout families in >1 ProxMiner families: "; 
	foreach $i (@temp){
		print OUTFILEDATA "$i, "; 
	}
	print OUTFILEDATA "\n"; 
...
if ($summflag){
	...
	print OUTFILEDATA "\n\#For unannotated ProxMiner families\n";
	print OUTFILEDATA "\#Size\tNofPMfams\n";
	foreach $i (sort {$a <=> $b} (keys (%size_counts))){
		if($i>1){ # to skip ccomps of size 1, nodes with self edges
			print OUTFILEDATA "$i\t$size_counts{$i}\n";
	...
	print OUTFILEDATA "\n\#For fully annotated ProxMiner families\n";
	print OUTFILEDATA "\#Size\tNofPMfams\n";
	foreach $i (sort {$a <=> $b} (keys (%size_counts))){
		if($i>1){ # to skip ccomps of size 1, nodes with self edges
			print OUTFILEDATA "$i\t$size_counts{$i}\n";
	...
	print OUTFILEDATA "\n\#For partially annotated ProxMiner families\n";
	print OUTFILEDATA "\#Size\tNofPMfams\n";
	foreach $i (sort {$a <=> $b} (keys (%size_counts))){
		if($i>1){ # to skip ccomps of size 1, nodes with self edges
			print OUTFILEDATA "$i\t$size_counts{$i}\n";
	...
	print OUTFILEDATA "\#Class\tNofPMfams\n";
	while ( ($i,$j) = each %CComp_annot_counts ){
			print OUTFILEDATA "$i\t$j\n";
	}
	
	print OUTFILEDATA "\n\# ProxMiner families with RScout families having annots from RTRPs and DNATPs: ";
	...
	print OUTFILEDATA "\n\#Class\tNofRScoutSingletons\n";
	$k=0;
	while ( ($i,$j) = each %snglton_annots ){
			print OUTFILEDATA "$i\t$j\n";
			if($i ne 'TBA'){ $k=$k+$j;}
	}
	print OUTFILEDATA "\n\# NOTE: TBA are novel or unannotated RScout singletons";
	print OUTFILEDATA "\n\# Number of annotated RScout singletons: $k\n";
}

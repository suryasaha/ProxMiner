...
# TODO: Avg no of fams in a connected comp, list of nof comps: comp size
# TODO: No of novel, extensions and completely known components
# TODO: CONSEQ LENGTHS FOR NON-ANNOTATED FAMILIES, COUNTS OF A RELATIONSHIP
...
foreach $i (@m_f_itemsets_table){
	if(($i->[8] > $UD_conf_lim) && ($i->[1] =~ /^U/)){ # for U relationships
		...
		if(!$UDgraph->has_edge($i->[0],$i->[2])) {
			...
			# check if fam2 U fam1 or fam1 D fam2 exists, take the higher one
	...
	if ($#$i > 0){
		...
		foreach $j (@$i){
			...
			if (exists $rmrb_annots{$j}){
				$k=$rmrb_annots{$j};
				@temp=@$k;
				print OUTFILEDATA "\#\tRMRB_Annot(Per_Divg, St, End, Rpt_Name\#Class, Strand(def:\+)), AnnotDB\n";

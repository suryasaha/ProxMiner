...
# v13 : Update counts using reciprocal relationships from the opposite strand.
# v13 : Added in the Sum relationship for counting all relationships between 2 fams
...
unless (@ARGV == 3){
	print "USAGE: $0 <config file> <RM .out file> <write copies??(-nocop/-cop)>\n";
	exit;
}
...
%pos_rel_history, %comp_rel_history, %both_rel_history, %pos_relationships_tot, %comp_relationships_tot, %both_relationships_tot,$tot_fams,
...
@ups_rels, @dns_rels, @ovlap_rels, %reci_rels, $rel);
...
foreach(@ups_rels) {print STDERR "$_->[0] $_->[1] $_->[2] $_->[3] $_->[4]\n";}
print STDERR "\n";
print STDERR "Downstream relationships\n";
foreach(@dns_rels) {print STDERR "$_->[0] $_->[1] $_->[2] $_->[3] $_->[4]\n";}
print STDERR "\n";
...
# @ups_rels, @dns_rels, @ovlap_rels: Dir	Rel	Rng Strt	Rng End	Reci Rel
...
while( ($i,$j) = each %pos_relationships){
	...
	if($temp[2]=~ /^[ud]/){
		...
		if (exists $comp_relationships{"$temp[1] $temp[0] $reci_rels{$temp[2]}"}) {
			$j+=$comp_relationships{"$temp[1] $temp[0] $reci_rels{$temp[2]}"};
		}
...
while( ($i,$j) = each %comp_relationships){
	...
	# only for U & D rels
	if($temp[2]=~ /^[ud]/){
		# if a reci rel exists for this family pair in the orig %pos_relationships
		if (exists $temphash{"$temp[1] $temp[0] $reci_rels{$temp[2]}"}) {
			$j+=$temphash{"$temp[1] $temp[0] $reci_rels{$temp[2]}"};
		}
...
# NA for both_relationships hash

# TOTAL REL COUNT FOR EACH PAIR
# %pos_relationships_tot : [fam1 fam2] = count
# %comp_relationships_tot : [fam1 fam2] = count
# %both_relationships_tot : [fam1 fam2] = count


# adding in a new total relationship with count for pos strand
while( ($i,$j) = each %pos_relationships){
	
	@temp=split(' ',$i);

	# increment relationship count or create relationship entry
	if (!exists $pos_relationships_tot{"$temp[0] $temp[1]"}) {
		$pos_relationships_tot{"$temp[0] $temp[1]"} = $j;
	}
	else{
		$pos_relationships_tot{"$temp[0] $temp[1]"}+=$j;
	}
...
# adding in a new total relationship with count for comp strand
while( ($i,$j) = each %comp_relationships){
	
	@temp=split(' ',$i);

	# increment relationship count or create relationship entry
	if (!exists $comp_relationships_tot{"$temp[0] $temp[1]"}) {
		$comp_relationships_tot{"$temp[0] $temp[1]"} = $j;
	}
	else{
		$comp_relationships_tot{"$temp[0] $temp[1]"}+=$j;
	}
...
# adding in a new total relationship with count for both strand
while( ($i,$j) = each %both_relationships){
	
	@temp=split(' ',$i);

	# increment relationship count or create relationship entry
	if (!exists $both_relationships_tot{"$temp[0] $temp[1]"}) {
		$both_relationships_tot{"$temp[0] $temp[1]"} = $j;
	}
	else{
		$both_relationships_tot{"$temp[0] $temp[1]"}+=$j;
	}
...
while( ($i,$j) = each %pos_relationships_tot){
	@temp=split(' ',$i);
	$rec=&get_index($temp[0]);
	print OUTFILEDATA "$temp[0]\t$counts[$rec][1]\t$counts[$rec][2]\t";
	$rec=&get_index($temp[1]);
	print OUTFILEDATA "$temp[1]\t$counts[$rec][1]\t$counts[$rec][2]\t";
	print OUTFILEDATA "$j\t+\tSum\n";
}
#calculating time taken
($user_t,$system_t,$cuser_t,$csystem_t) = times;
...
while( ($i,$j) = each %comp_relationships_tot){
	@temp=split(' ',$i);
	$rec=&get_index($temp[0]);
	print OUTFILEDATA "$temp[0]\t$counts[$rec][1]\t$counts[$rec][2]\t";
	$rec=&get_index($temp[1]);
	print OUTFILEDATA "$temp[1]\t$counts[$rec][1]\t$counts[$rec][2]\t";
	print OUTFILEDATA "$j\tC\tSum\n";
}
#calculating time taken
($user_t,$system_t,$cuser_t,$csystem_t) = times;
...
# printing relationships on both strands
while( ($i,$j) = each %both_relationships){
	@temp=split(' ',$i);
	$rec=&get_index($temp[0]);
	print OUTFILEDATA "$temp[0]\t$counts[$rec][1]\t$counts[$rec][2]\t";
...
while( ($i,$j) = each %both_relationships_tot){
	@temp=split(' ',$i);
	$rec=&get_index($temp[0]);
	print OUTFILEDATA "$temp[0]\t$counts[$rec][1]\t$counts[$rec][2]\t";
	$rec=&get_index($temp[1]);
	print OUTFILEDATA "$temp[1]\t$counts[$rec][1]\t$counts[$rec][2]\t";
	print OUTFILEDATA "$j\tB\tSum\n";
}
#calculating time taken
($user_t,$system_t,$cuser_t,$csystem_t) = times;

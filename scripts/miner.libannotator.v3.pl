...
unless (@ARGV == 3){
	print "USAGE: $0 <rep lib fasta file> <.cat file from RM> <annotation threshold%>\n";
	exit;
}

my ($ifname1,$ifname2,$rec,@temp,@cat_table,@faminfo,$DNA,$thold,
$ctr,$i,$j,$tot_recs,$fams_annotated,$tot_seqs,$user_t,$system_t,$cuser_t,$csystem_t,
$tmpdata,@fam_annot);
...
# 	48930 4.39 0.18 0.16 R=1 4984 10670 (0) C RIRE8B_I#LTR/Gypsy (0) 5981 294 5
	...
	if ($annot_len > int(ceil($thold * $ref_len))){
		...
		print OUTFILEDATA "$ref_fam  $ref_len  $ctr $annot_ratio\n";
		...
		foreach (@fam_annot){
			...
			if ($_->[8] eq 'C'){
				...
				print OUTFILEDATA "$ref_fam $_->[0] $_->[1] $_->[2] $_->[3] $_->[4] $_->[5] $_->[6] $_->[7] $_->[8] $_->[9] $_->[10] $_->[11] $_->[12]\n";
			}
		}
		print OUTFILEDATA "\n";
	}

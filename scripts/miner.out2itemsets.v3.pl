...
# 70% + (Ovlap>70)
# Creating the itemsets in the format
...
# @itemsets : fam1, fam1-count, fam1-avglen, fam2, fam2-count, fam2-avglen, Occurence, Category
#sorting @itemsets on fam1, fam2 in ascending and occurences in decreasing order
...
foreach $i (1 .. $#itemsets){#skipping the 1st record
	my ($flag,$rel, $ref_fam1,$ref_fam2,$ref_occ,$sub_fam1,$sub_fam2,$sub_occ);
	...
	if ($rel eq "u1") { $rel="d1";}
	elsif ($rel eq "u2") { $rel="d2";}
	elsif ($rel eq "u3") { $rel="d3";}
	elsif ($rel eq "d1") { $rel="u1";}
	elsif ($rel eq "d2") { $rel="u2";}
	elsif ($rel eq "d3") { $rel="u3";}
